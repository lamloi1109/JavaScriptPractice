...
export function isIncreasingNumberByDistance(number, distance) {
  // Check args
  if (
  ...
  while (remainingNumber > 0) {
    ...
    if (prevNumber < 10 && prevNumber - lastNumber > distance) {
      return false;
    }
...
export function isIncreasingNumberByDistanceV2(number, distance) {
  // Check args
  if (
    typeof number !== "number" ||
    typeof distance !== "number" ||
  ...
  for (let index = 1; index < numberStringList.length; index++) {
    ...
    if (numberStringList[index] - numberStringList[index - 1] > distance)
      return false;
  }
  return true;
}
